...
async function put_s3(event){
    ...
    //const cmdline = "./ffmpeg -y -i " + srcPath + " -ar 16000 -ac 1 " + dstPath; //del 20181221
    const cmdline = "./ffmpeg -y -i " + srcPath + " -af pan=\"mono|c0=c0\" -ar 44000 -ac 1 " + dstPath;//upd 20181221
    const cmdline2 = "./ffmpeg -y -i " + srcPath + " -af pan=\"mono|c0=c1\" -ar 44000 -ac 1 " + dstPath2;//add 20181221
...
res.on("end", function(res){
	...
	for(var i=0; i<res.length; i++){
			console.log(res[i]);
			try{
					message2 += res[i].result.sentence[0]['surface'];
			}
			catch(error){};
	}
	//ログ確認
